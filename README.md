...
| 长时业务编排难落地 | `EasyCoreDaprWorkflow` + `IEasyCoreWorkflowClient` |
...
│                                                             │
│   EasyCoreDaprActors → 官方 Actors（单线程 turn）         │
...
builder.EasyCoreDapr()
    .EasyCoreDaprAll(); // 或按需 EasyCoreDaprInvocation / State / PubSub ...
...
| 方法 | 作用 | 图标语义 |
|---|---|---|
| `EasyCoreDapr` | Options、Sidecar 客户端、健康等待 | 🧱 |
| `EasyCoreDaprInvocation` | `IDaprInvoker` | 📞 |
| `EasyCoreDaprState` | `IDaprStateStore`（含 ETag） | 🗄️ |
| `EasyCoreDaprPubSub` | `IDaprPubSub` | 📣 |
| `EasyCoreDaprSecrets` | `IDaprSecrets` | 🤫 |
| `EasyCoreDaprBindings` | `IDaprBinding` | 🔗 |
| `EasyCoreDaprAll` | 以上 HTTP 能力一次注册 | 📦 |
| `EasyCoreDaprActors` | Actors 运行时 + `RegisterActor<T>()` | 🎭 |
| `EasyCoreDaprWorkflow` | Workflow 宿主 + `IEasyCoreWorkflowClient` | 🔄 |
